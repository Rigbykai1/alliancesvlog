...
const CarrouselCard = ({
  ...
  return (
    <>
      ...
      >
        <button
          className="hidden h-full items-center rounded-lg px-4 border opacity-70 bg-base-100
                    sm:flex
                    hover:cursor-pointer hover:opacity-100
                    disabled:border-transparent disabled:bg-neutral disabled:opacity-50 disabled:cursor-default"
          onClick={handlePrev}
          disabled={page === 0}
        >
          <TfiAngleLeft />
        ...
        >
          {paginated.map((ocupation, idx) => (
            ...
            >
              ...
              <h4 className="text-md uppercase font-semibold text-accent">
                {ocupation.title1}
              </h4>
              <p className="text-md max-w-sm">{ocupation.description}</p>
              <p className="text-accent text-sm">{ocupation.year}</p>
            </div>
          ))}
        </div>
        <button
